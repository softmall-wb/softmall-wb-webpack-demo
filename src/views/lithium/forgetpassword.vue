  ...
  <div wb:style-var-bind>
    ...
    >
      <ul class="forget-password-form">
        ...
        <!-- 验证码 -->
        <li class="form-item form-input" v-if="true">
          <div class="icon">
            <icon-base :width="iconSize" :height="iconSize" iconColor>
              <captcha-icon />
            </icon-base>
          ...
          <button
            id="get-captcha"
            @click="sendCode"
          >{{!forgetService.able_to_send_vcode? forgetService.vcode_second: '获取验证码'}}</button>
        </li>
        <!-- 验证码 -->
...
export default {
  ...
  data() {
    ...
    const forgetService = new ForgetService(
      true,
      phoneCheck,
      passwordCheck,
      confirmCheck,
  ...
  methods: {
    ...
    sendCode() {
      if (!this.forgetService.able_to_send_vcode) return;
