<template>
  <div wb:style-var-bind>
    <div :style="{'background-image': 'url('+ theme_background_img+ '@w_750,h_750)'}" class="login">
      <div class="logo_title">欢迎登录{{ mall_name }}</div>
      <ul class="login-form">
        <div class="tabbar">
          <div
            @click="currentLoginMode = 'password'"
            :class="{'active':(currentLoginMode == 'password')}"
            class="tabbar_item"
          >密码登录</div>
          <div
            @click="currentLoginMode = 'captcha'"
            :class="{'active':(currentLoginMode == 'captcha')}"
            class="tabbar_item"
          >验证码登录</div>
          <div
            :style="{left: (currentLoginMode == 'password' ? '0' : '50%')}"
        ...
        <!-- 密码 -->
        <li class="form-item form-input" v-if="currentLoginMode == 'password' || visible_password">
          <div class="icon">
            <icon-base :width="iconSize" :height="iconSize" iconColor>
              <password-icon />
            </icon-base>
        ...
        <!-- 验证码 -->
        <!-- <li class="form-item form-input" v-if="loginService.vcode_force"> -->
        <li class="form-item form-input" v-if="currentLoginMode == 'captcha'">
          <div class="icon">
...
.tabbar {
  ...
  border-radius: 100px;
  background-color: #fff;
  -moz-box-shadow: 0px 6px 8px #fef4e7;
  -webkit-box-shadow: 0px 6px 8px #fef4e7;
  box-shadow: 0px 6px 8px #fef4e7;
  position: relative;
  .tabbar_item {
    width: 50%;
    text-align: center;
  ...
  .background_block {
    ...
    left: 50%;
    border-radius: 100px;
    background-color: #fd8b45;
    background-image: linear-gradient(135deg, #fd8b45 0%, #fe6657 100%);
    -moz-box-shadow: 0px 6px 8px #fef4e7;
    -webkit-box-shadow: 0px 6px 8px #fef4e7;
    box-shadow: 0px 6px 8px #fef4e7;
    transition: 0.3s;
